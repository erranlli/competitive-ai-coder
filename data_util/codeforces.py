...
def get_record_by_problem_id(ds, problem_id: str) -> Optional[Dict[str, Any]]:
    try:
        for record in ds:
...
def find_records_by_contest(ds, contest_id) -> List[Dict[str, Any]]:
    try:
...
def list_available_problem_ids(ds, limit: int = 50) -> List[str]:
    problem_ids = []
...
def search_problems_by_title(ds, title_keyword: str, limit: int = 10):
    results = []
...
def display_problem_info(record: Optional[Dict[str, Any]]) -> None:
    if not record:
...
def quick_problem_lookup(ds, problem_id: str):
...
def pretty_print_codeforces_problem_dark(problem_data: dict) -> None:
...
def pretty_print_codeforces_problem(problem_data: dict) -> None:
    if not isinstance(problem_data, dict):
